...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        self.title = @"Select Run-n-Read Device";
        self.isWebView = NO;
        pageNum = 1;
    }
    return self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cell.textLabel.text = [NSString stringWithFormat:@"%@",p.name];     // Peripheral name was showing up as TI BLE Sensor Tag
    cell.textLabel.text = [NSString stringWithFormat:@"Run-n-Read Device"];
...
-(NSString *) tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    if (section == 0) {
        if (self.sensorTags.count > 1 )return [NSString stringWithFormat:@"%d Run-n-Read Devices Found",self.sensorTags.count];
        else return [NSString stringWithFormat:@"%d Run-n-Read Device Found",self.sensorTags.count];
    }
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(self.isForCalibration) {
        ...
        //CalibrationViewController *vC = [[CalibrationViewController alloc]initWithStyle:UITableViewStyleGrouped andSensorTag:d];
        AccelerometerGraph *vC = [[AccelerometerGraph alloc] init:d];
        [self.navigationController pushViewController:vC animated:YES];
    } else {
        RunReadPNGViewController *rrC = [[RunReadPNGViewController alloc] init:d];
        if(self.isWebView) {
            [rrC setWebView:YES withURL:webURL];
        } else {
            [rrC setStoryNumber:storyNum withPages:maxPages];
            [rrC setPageNumber:pageNum];
        }
...
-(NSMutableDictionary *) makeSensorTagConfiguration {
    ...
    [d setValue:@"16" forKey:@"Accelerometer period"];
    ...
    [d setValue:@"16" forKey:@"Magnetometer period"];
